...
# Usage: rake post title="我是中国人，我爱我的家人和我的祖国" category="life" tags="抒情 love" date="2012-02-09"
...
desc "以当前的日期在images目录下创建目录用来存放图片资源, 支持指定日期 date='2015-09-20'"
task :imgpath do
    begin
      date = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y%m%d')
    rescue Exception => e
    ...
    filename = File.join('images', date)
    if !File.directory?(filename)
        mkdir_p filename
    end
end
